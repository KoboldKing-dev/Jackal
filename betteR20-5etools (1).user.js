...
const folder=d20plus.journal.makeDirTree(`Monsters`,folderName);const path=["Monsters",...folderName,data._displayName||data.name];if(!d20plus.importer._checkHandleDuplicate(path,overwrite))return;const name=data.name;const pType=Parser.monTypeToFullObj(data.type);const renderer=new Renderer();renderer.setBaseUrl(BASE_SITE_URL);const fluff=Renderer.monster.getFluff(data,monsterFluffData[data.source]);let renderFluff=null;if(fluff){const depth=fluff.type==="section"?-1:2;if(fluff.type!=="section")renderer.setFirstSection(false);renderFluff=renderer.render({type:fluff.type,entries:fluff.entries},depth);}
